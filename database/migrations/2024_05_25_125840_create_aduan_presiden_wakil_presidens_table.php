...
class CreateAduanPresidenWakilPresidensTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
{
    Schema::create('aduan_presiden_wakil_presidens', function (Blueprint $table) {
        $table->id();
        $table->enum('status', ['queue', 'proces', 'done'])->default('queue');
        $table->string('jenis_atribut');
        $table->string('jenis_pemilihan')->default('Aduan Presiden dan Wakil Presiden');
        $table->string('nama_calon');
        $table->string('lokasi_pemasangan');
        $table->string('nama_jalan'); // Kolom untuk menyimpan nama jalan
    ...
    {
        Schema::dropIfExists('aduan_presiden_wakil_presidens');
    }
}
