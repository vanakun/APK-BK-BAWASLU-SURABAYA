...
class CreateAduanDpdRiTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('aduan_dpd_ri', function (Blueprint $table) {
            $table->id();
            $table->enum('status', ['queue', 'proces', 'done'])->default('queue');
            $table->string('jenis_atribut');
            $table->string('jenis_pemilihan')->default('Aduan DPD - RI');
    ...
    {
        Schema::dropIfExists('aduan_dpd_ri');
    }
}
