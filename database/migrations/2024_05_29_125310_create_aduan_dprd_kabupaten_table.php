...
class CreateAduanDprdKabupatenTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('aduan_dprd_kabupaten', function (Blueprint $table) {
            $table->id();
            $table->enum('status', ['queue', 'proces', 'done'])->default('queue');
            $table->string('jenis_atribut');
            $table->string('jenis_pemilihan')->default('DPRD - KABUPATEN/KOTA');
            $table->string('nama');
    ...
    {
        Schema::dropIfExists('aduan_dprd_kabupaten');
    }
}
