...
class CreateAduanDprdProvinsiTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('aduan_dprd_provinsis', function (Blueprint $table) {
            $table->id();
            $table->enum('status', ['queue', 'proces', 'done'])->default('queue');
            $table->string('jenis_atribut');
            $table->string('jenis_pemilihan')->default('DPRD - PROVINSI');
            $table->string('nama');
    ...
    {
        Schema::dropIfExists('aduan_dprd_provinsis');
    }
}
