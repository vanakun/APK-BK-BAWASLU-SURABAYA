...
class AduanDprRiController extends Controller
{
    public function index()
    {
        $aduan = AduanDprRi::all();
        return view('aduan_dpr_ri.index', compact('aduan'));
    }
    ...
    public function create()
    {
         ...
         if ($activeYear) {
            $presiden = CalonDprRi::whereHas('tahunPemilihan', function($query) use ($activeYear) {
                $query->where('tahun_pemilihan', $activeYear->tahun_pemilihan_aktif);
            })->get();
        } else {
            $presiden = collect();
        }
        $calonPartai = CalonDprRi::with('partai')->get();
        //dd($calonPartai);
        return view('pages.user.aduan.dpr-ri.create',compact('calonPartai','presiden', 'activeYear'));
    }
    ...
    public function store(Request $request)
    {
        ...
        AduanLog::create([
            ...
            'aduan_id' => $aduan->id,
            'aduan_type' => AduanDprRi::class,  // Tipe aduan, bisa diubah sesuai kebutuhan
        ]);
    
        return redirect()->route('tenagaahliDashboard')->with('success', 'Aduan berhasil ditambahkan.');
    }

    public function show(AduanDprRi $aduanDprRi)
    {
        return view('aduan_dpr_ri.show', compact('aduanDprRi'));
    }

    public function edit(AduanDprRi $aduanDprRi)
    {
        return view('aduan_dpr_ri.edit', compact('aduanDprRi'));
    }

    public function update(Request $request, AduanDprRi $aduanDprRi)
    {
        $validatedData = $request->validate([
            'jenis_atribut' => 'required|string|max:255',
            'nama_calon_dpd' => 'required|string|max:255',
            'nama_partai' => 'required|string|max:255', // Validasi untuk nama partai
            'lokasi_pemasangan' => 'required|string|max:255',
            'nama_jalan' => 'nullable|string|max:255',
            'tanggal_laporan' => 'required|date',
            'tanggal_akhir_ditertibkan' => 'required|date',
            'gambar_sebelum' => 'nullable|image|max:2048',
            'tahun_pemilihan_id' => 'required|integer|exists:tahun_pemilihans,id',
        ]);
        ...
        $aduanDprRi->update($validatedData);

        return redirect()->route('aduan_dpr_ri.index')->with('success', 'Aduan berhasil diperbarui.');
    }

    public function destroy(AduanDprRi $aduanDprRi)
    {
        $aduanDprRi->delete();
        return redirect()->route('aduan_dpr_ri.index')->with('success', 'Aduan berhasil dihapus.');
    }
    public function indexAntrianDPR()
    {
        $aduandprqueue = AduanDprRi::where('status', 'queue')->paginate(10);
        
        $aduandprproces = AduanDprRi::where('status', 'proces')->paginate(10);

        $aduandprdone = AduanDprRi::where('status', 'done')->paginate(10);
        return view('pages.admin.aduan-dpr.index', compact('aduandprdone','aduandprqueue','aduandprproces'));
    }
    public function insertqueuedpr($id)
    {
        $aduan = AduanDprRi::findOrFail($id);
        return view('pages.admin.aduan-dpr.insertqueue', compact('aduan'));
    }

    public function updatequeuedpr(Request $request, $id)
    {
        ...
        return redirect()->route('indexAntrianDPR');
    }

    public function editaduandpr($id)
    {
        $aduan = AduanDprRi::findOrFail($id);
        return view('pages.admin.aduan-dpr.edit', compact('aduan'));
    }

    public function updateAduandpr(Request $request, $id)
    {
        // Validasi data yang diterima dari formulir
        $validatedData = $request->validate([
            'jenis_atribut' => 'required',
    ...
    public function cetak(Request $request)
    {
        ...
        $aduans = AduanDprRi::where('tahun_pemilihan_id', $tahunPemilihan)->get();
        return view('pages.admin.aduan-dpr.cetakpdf', compact('aduans', 'tahunPemilihan'));
    }
